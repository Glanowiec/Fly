...
public class UpdateTerrainHeight : MonoBehaviour {
	...
	void AddShip()
	{
		 ...
		 float pz = c.transform.position.z + 1100;
		 if (l <=3) 
		 {
			Transform obj = pulaZeppelin.getObject();
			obj.transform.position = new Vector3(UnityEngine.Random.Range (900,1100) ,UnityEngine.Random.Range(30,70),pz);
			obj.Rotate(new Vector3(270.0f,90.0f,0.0f));
			onScene.Enqueue(obj);
		 ...
		 if (l < 7)
		{
			...
			obj.transform.position = new Vector3(UnityEngine.Random.Range (950,1050) ,15,pz);
			obj.Rotate(new Vector3(270.0f,UnityEngine.Random.Range(0,180.0f),0.0f));
		    onScene.Enqueue(obj);
		} else
		{
			if (bridge.transform.position.z < c.transform.position.z)
			{
				GameObject t = GameObject.Find("Bridge/tank");
				t.SendMessage("resetHit");
				bridge.transform.position = new Vector3(1000, UnityEngine.Random.Range (40,80), pz);
				Vector3 v = bridge.transform.position;
				v.x = 600;
				t.transform.position = v;
				t.GetComponent<Rigidbody>().velocity = new Vector3(5,0,0);
			} else
			{
				Transform obj = pulaShip.getObject ();
				obj.transform.position = new Vector3(UnityEngine.Random.Range (950,1050) ,15,pz);
	...
	void UpdatePos()
	{
	   ...
	   {	
		...
		dt = DateTime.Now;
		    ...
		    if (c.transform.position.z >= (TerrainC.transform.position.z + 1000))
			{
			...
			// check if eobject must be returned to pool
			if (onScene.Count > 0){
				Transform o = (Transform)onScene.Peek();
			
				if ( o != null)
				{
					if (c.transform.position.z > o.transform.position.z)
					{
